...
def compiler(task_id):
    ...
    if request.method == "POST":
        ...
        if now <= expiry:
            # Generate unique filename for website stability
                ...
                if output == task[3].strip():
                    ...
                    if not already:
                        c.execute("INSERT INTO attendance(student_id,date) VALUES(?,?)",
                                  (session["student_id"], today))
                        conn.commit()
                        result += "\n\n✅ Correct! Attendance Marked."
            except Exception as e:
                result = "Error: " + str(e)
            finally:
                # Cleanup: Delete the temp file after execution
...
def report():
    ...
    if request.method=="POST":
        ...
        for s in students:
            ...
            fine=absents*25
            data.append((s[1],s[2],presents,absents,round(percent,2),fine))
    conn.close()
